...
- Frontend `api/*.js` files stay thin. Matching logic and similar behavior live in the page or a dedicated helper. Shared pantry/recipe units live in `frontend/src/units.js` (`COMMON_UNITS`); add forms use a required `<select>`, not free text. Older custom units can still appear as an extra option until changed. Pantry quantity edits use `updateIngredient(id, { quantity })` against the partial PUT above. Cook uses `cookRecipe(id)` → `POST /recipes/{id}/cook`. Suggest uses `getSuggestions()` → `GET /recipes/suggest` with a 90s timeout; loading copy, error mapping, and Try again live in `Pantry.jsx`.
