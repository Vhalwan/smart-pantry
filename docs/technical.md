...
- Frontend `api/*.js` files stay thin. Matching logic and similar behavior live in the page or a dedicated helper. Pantry quantity edits use `updateIngredient(id, { quantity })` against the partial PUT above. Cook uses `cookRecipe(id)` → `POST /recipes/{id}/cook`. Suggest uses `getSuggestions()` → `GET /recipes/suggest` with a 90s timeout; loading copy, error mapping, and Try again live in `Pantry.jsx`.
