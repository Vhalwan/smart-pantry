...
frontend/
  src/
    pages/           # Tonight, Pantry, Recipes, MealPlans, Login, Register
...
`reason` is `missing` or `unit_mismatch`. A recipe with no ingredient lines returns 200 with empty arrays (the Recipes page disables Cook this and does not call the API). Unknown recipe: 404. Cook math stays in the route, not in a loop of frontend PUTs. The Recipes page shows a short card note (same tone as the save skip note) and refetches ingredients. After a response with any `updated` or `short` line, that card’s Cook this stays disabled as Cooked and shows View pantry (`/pantry`) until the page remounts. An all-`skipped` result does not lock the button. Cook does not use `pendingIngredientRemovals`. The Pantry page shows a calm note on rows whose quantity is 0 (cook leftovers; the stepper still removes at 0 instead of leaving a 0 row). Meal Plans reuse the same cook endpoint for rows whose `planned_date` is today (local calendar day); readiness is computed client-side in `cookHelpers.js` with the same alias rules for **every** plan row. Linked-ingredient expiry flags use `expiryHelpers.js` (same 3-day window as Pantry). Future meal-plan rows have no Cook this. Tonight (`/tonight`, `Tonight.jsx`) reuses the same three GETs in parallel (ingredients, recipes, meal plans) and the same helpers: Use it up from `getExpiryStatus` (3-day window), today’s meal plans via `localTodayISO`, readiness + linked expiry + Cook this for those rows only. No new API. Saved recipes are grouped into Ready to cook, Almost ready (short), and Need attention (blocked) via `assessCookReadiness`; each row has Cook this with the same behavior as Recipes. When nothing is ready or almost ready, a CTA links to Pantry / Suggest. Short and blocked recipes also feed `collectRecipeGaps` in `cookHelpers.js`: one plain row per short, missing, or unit-mismatched line (recipe name + amount/unit when known; identical text deduped). Tonight shows that as **What’s missing** (hidden when empty). No persistence and no shopping-cart UI.
...
- Frontend `api/*.js` files stay thin. Matching logic and similar behavior live in the page or a dedicated helper. Shared pantry/recipe units live in `frontend/src/units.js` (`COMMON_UNITS`, aliases via `canonicalUnit` / `unitsMatch`); add forms use a required `<select>`, not free text. Older custom units can still appear as an extra option until changed. Backend cook uses the same alias table in `app/units.py`. Pantry quantity edits use `updateIngredient(id, { quantity })` against the partial PUT above. Cook uses `cookRecipe(id)` → `POST /recipes/{id}/cook` (Recipes, today’s Meal Plans, and today’s rows on Tonight). Suggest uses `getSuggestions()` → `GET /recipes/suggest` with a 90s timeout; loading copy, error mapping, and Try again live in `Pantry.jsx`. Cook readiness preview lives in `frontend/src/cookHelpers.js`. Tonight is `frontend/src/pages/Tonight.jsx` at `/tonight` (nav first in `AppLayout.jsx`). Tonight loads ingredients, recipes, and meal plans with `Promise.allSettled`: one failed GET does not blank the others; empty-pantry CTAs only after a successful pantry GET; all-failed uses a page-level Try again; a slow load (3s) shows a waking-up note.
...
- Pantry-list expiry status is computed in `expiryHelpers.js` (shared with Meal Plans): parse `YYYY-MM-DD` as a local calendar date, compare to today with UTC day numbers (time-of-day and DST safe), and show Expired / Expiring soon when past or within `NEAR_EXPIRY_DAYS` (3). The suggest route uses the same 3-day idea on the backend (`NEAR_EXPIRY_DAYS` in `suggestions.py`, `date.today()` vs `expiry_date`) to tag prompt lines; there is still no dedicated expiry API field. Meal Plans call `assessLinkedExpiry` to name linked expired / expiring-soon ingredients on each plan. Tonight also uses `collectExpiringPantryIds` and `recipeUsesExpiringPantry` to badge Ready / Almost ready recipes whose linked `ingredient_id`s overlap that same window.
...
- 2 Sep 2026: v2 start — `/tonight` (`Tonight.jsx`), Tonight first in nav. Parallel GET ingredients/recipes/meal-plans. Use it up + today’s plan Cook this. No new API.
- 3 Sep 2026: Tonight recipe groups (Ready / Almost ready / Need attention) with Cook this; nothing-ready CTA. No new API.
- 6 Sep 2026: Tonight gaps list — `collectRecipeGaps` in `cookHelpers.js`, **What’s missing** on `Tonight.jsx`. No new API.
- 7 Sep 2026: Tonight Ready / Almost ready — **Uses expiring items** badge via `collectExpiringPantryIds` / `recipeUsesExpiringPantry` in `expiryHelpers.js`. No new API.
- 8 Sep 2026: Tonight load — `Promise.allSettled` on ingredients/recipes/meal-plans; per-section or all-failed Try again; waking-up note after 3s. No new API.
