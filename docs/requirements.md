...
| FR-12 | A user can create, view, update, and delete meal plans that point at their saved recipes (a date plus breakfast, lunch, or dinner). For each plan, the app can show pantry readiness (Ready / short / mismatch) and flag linked ingredients that are expired or expiring soon. For a plan dated today, the user can Cook this (same pantry update as from Recipes). |
...
FR-8 through FR-10 are in the UI (save from a suggestion card with name matching, unit matching including aliases, and a skip note that says what to do next; matching also covers simple last-word singular/plural). FR-12: Meal Plans list each row with Ready / short / mismatch plus Expired / Expiring soon names when linked pantry items need attention; Cook this only for today (same cook API; future dates stay plans-only for cook). FR-4’s optional category and expiry are collected on the Pantry add form and shown in the list; unit is a required select of common kitchen measures (not free text). FR-5: delete is refused with a named-recipe message when the item is still linked to a recipe (including after the pantry stepper hits 0). FR-13 is in the UI (quantity stepper / direct edit via partial PUT). Quantity at 0 on the stepper auto-removes the item after a short Undo window (delayed DELETE); the explicit Delete control remains immediate. FR-14 is in the UI (calm per-row Expired / Expiring soon labels next to the expiry date; client-side calendar-day compare, 3-day near window; those rows sort to the top of the list). FR-15 is in the suggestion prompt (backend tags the same 3-day / expired window and asks Gemini to prefer those items when it reasonably can; also rush-friendly / thin-pantry wording). An empty pantry does not call the model; after login the pantry shows a first-use note (add a few ingredients, then Suggest recipes) and Suggest stays off. FR-16 is in the UI: Cook this on a saved recipe card (and on today’s meal-plan row) updates pantry quantities (clamp at 0, keep linked rows at 0, report short or skipped lines). After a subtract, the button stays Cooked on that card/row with a View pantry link; an all-skipped result stays retryable. Pantry rows at 0 after cook show that they are still on a recipe. Not on suggestion cards or future meal plans. NFR-6: a slow Suggest shows a waking-up note; a failed call shows a plain-language error and Try again (no silent retry, no raw error text).
...
- Cook this on meal-plan rows or on an unsaved suggestion for a **future** date (today’s meal-plan cook is in; future plans stay plans-only for cook)
- Safe amount conversion between different measures (aliases only in this version)
...
| 19 Aug 2026 | NFR-6: Suggest waking-up note, mapped failure copy, Try again. No new FR. Suggestion matching and cook unchanged. |
| 21 Aug 2026 | Empty pantry first-use: add-then-Suggest next step on the pantry. FR-10 skip notes already said what to do next. Unit on add/recipe lines is a required common-measure select (FR-4 UX). No new FR. |
...
| 8 Sep 2026 | v2 Week 2 day 3: Tonight load errors — failed fetch is Try again, not an empty-pantry helper. No new FR. |
