...
| FR-12 | A user can create, view, update, and delete meal plans that point at their saved recipes (a date plus breakfast, lunch, or dinner). For a plan dated today, the app can show whether the pantry can cover that recipe and let the user Cook this (same pantry update as from Recipes). |
...
- Cook this on meal-plan rows or on an unsaved suggestion for a **future** date (today’s meal-plan cook is in; future plans stay plans-only)
